...
public static class EnsExtensions
{
		...
		this GlobalContext globalContext, byte[] ensNameHash, HexBigInteger blockNumber)
	{
		...
		try
		{
			forwardResolverAddress = await globalContext.Policy.ExecuteAsync(() => globalContext.Services.Web3.Eth.GetEnsService().ENSRegistryService
		...
		try
		{
			forwardAddress = await globalContext.Policy.ExecuteAsync(() => forwardResolver.AddrQueryAsync(ensNameHash, new BlockParameter(blockNumber)));
		}
		catch
		{
			return new ForwardResolutionResult
		...
		this GlobalContext globalContext, byte[] reverseAddressNameHash, HexBigInteger blockNumber)
	{
		...
		string? reverseResolverAddress = await globalContext.Policy.ExecuteAsync(() => globalContext.Services.Web3.Eth.GetEnsService().ENSRegistryService
		...
		try
		{
			ensName = await globalContext.Policy.ExecuteAsync(() => reverseResolver.NameQueryAsync(reverseAddressNameHash, new BlockParameter(blockNumber)));
		}
		catch
		{
			return new EnsReverseResult
