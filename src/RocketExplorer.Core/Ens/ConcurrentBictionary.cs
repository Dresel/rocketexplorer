	...
	where T2 : notnull
{
	...
	public virtual T1 this[T2 key]
	{
		...
		set
		{
			...
			try
			{
				if (this.forward.TryGetValue(value, out T2? reverseValue) && !this.reverse.Comparer.Equals(reverseValue, key))
				{
	...
	public virtual T2 this[T1 key]
	{
		...
		set
		{
			...
			try
			{
				if (this.reverse.TryGetValue(value, out T1? reverseValue) && !this.forward.Comparer.Equals(reverseValue, key))
				{
					throw new InvalidOperationException("Value is already mapped to a different key.");
				}
