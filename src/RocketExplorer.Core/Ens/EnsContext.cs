...
public record class EnsContext
{
	...
	public static async Task<EnsContext> ReadAsync(
		...
		ProcessHistory processHistory, bool initialSync, ILogger<EnsContext> logger,
		CancellationToken cancellationToken = default)
	{
		...
		if (ensContext.InitialSync)
		{
			// Initial sync, clear all incremental processed addresses and add all known addresses
			processHistory.Clear();

			foreach (byte[] addressBytes in nodesContext.Nodes.Data.Index.Select(x => x.Value.ContractAddress))
			{
				await processHistory.AddAddressEnsCandidateAsync(addressBytes, cancellationToken: cancellationToken);
			}

			foreach (HolderEntry holder in tokensContext.RPLTokenInfo.Holders.Values)
			{
				await processHistory.AddAddressEnsCandidateAsync(
					holder.Address.HexToByteArray(), cancellationToken: cancellationToken);
			}
