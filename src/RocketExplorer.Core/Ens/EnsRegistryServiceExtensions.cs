...
public static class EnsRegistryServiceExtensions
{
		...
		this GlobalContext globalContext, byte[] ensNameHash, CancellationToken cancellationToken = default)
	{
		...
		if (result is not null)
		{
			globalContext.LoggerFactory.CreateLogger<EnsSync>().LogWarning(
				"Removed entries from reverse record {Ens} => {Address}", result.Value.RemovedEnsName, result.Value.RemovedAddress);

			await globalContext.Services.ProcessHistory.AddAddressEnsCandidateAsync(result.Value.RemovedAddress.HexToByteArray(), result.Value.RemovedEnsName, cancellationToken);
		}
	}
		...
		this GlobalContext globalContext, byte[] reverseAddressNameHash, CancellationToken cancellationToken = default)
	{
		...
		(string RemovedAddress, string RemovedEns)? result = context.TryRemoveFromReverseAddressNameHash(reverseAddressNameHash);

		if (result is not null)
		{
			globalContext.LoggerFactory.CreateLogger<EnsSync>().LogWarning(
				"Removed entries from reverse record {Address} => {Ens}", result.Value.RemovedAddress, result.Value.RemovedEns);

			await globalContext.Services.ProcessHistory.AddAddressEnsCandidateAsync(result.Value.RemovedAddress.HexToByteArray(), result.Value.RemovedEns, cancellationToken);
		}
	}
