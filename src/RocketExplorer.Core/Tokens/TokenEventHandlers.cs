...
public class TokenEventHandlers
{
		...
		CancellationToken cancellationToken = default)
	{
		...
		if (!fromAddress.IsTheSameAddress(AddressUtil.ZERO_ADDRESS))
		{
			...
			HolderEntry holderEntry = tokenInfo.Holders.TryGetValue(fromAddress, out HolderEntry? value) ? value : new HolderEntry()
			{
				Address = fromAddress,
				AddressEnsName = null,
				Balance = 0,
			...
			{
				tokenInfo.Holders[fromAddress] = holderEntry with
				{
					Balance = fromBalance,
				};
			}
		...
		if (!toAddress.IsTheSameAddress(AddressUtil.ZERO_ADDRESS))
		{
			if (!tokenInfo.Holders.ContainsKey(toAddress))
			{
				...
				await globalContext.Services.ProcessHistory.AddAddressEnsCandidateAsync(toAddress.HexToByteArray(), cancellationToken: cancellationToken);
